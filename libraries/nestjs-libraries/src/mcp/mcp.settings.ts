...
export class McpSettings {
  ...
  createServer(organization: string, service: MainMcp) {
    this._server = new McpServer(
      ...
      {
        instructions: `Socian is a service to schedule social media posts for ${socialIntegrationList
          .map((p) => p.name)
          .join(
            ', '
          )} to schedule you need to have the providerId (you can get it from SOCIAN_PROVIDERS_LIST), user need to specify the schedule date (or now), text, you also can send base64 images and text for the comments. When you get SOCIAN_PROVIDERS_LIST, always display all the options to the user`,
      }
    );
